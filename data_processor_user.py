...
class DataProcessor:
    ...
    def validate_match_input(self, user_data: Dict, policy_data: Dict) -> bool:
        ...
        try:
            ...
            policy_type = policy_data.get(
                "适用对象", "") or policy_data.get("类型", "")
            if policy_type and policy_type != "个人":
                return False
        ...
        try:
            ...
            if operator == 'between':
                if isinstance(condition_value, list) and len(condition_value) == 2:
                    ...
                    if user_val is None or min_val is None or max_val is None:
                        self.logger.warning(
                            f"between操作符类型转换失败: user={user_value}, range={condition_value}")
                        return False
            ...
            # 数字比较操作符
                ...
                if user_val is None or condition_val is None:
                    self.logger.warning(
                        f"数字比较类型转换失败: user={user_value}, condition={condition_value}")
                    return False
            ...
            # 字符串比较
                ...
                if user_str is None or condition_str is None:
                    self.logger.warning(
                        f"字符串比较转换失败: user={user_value}, condition={condition_value}")
                    return False
            ...
            self.logger.error(
                f"条件检查异常: user={user_value}, operator={operator}, condition={condition_value}, 错误: {e}")
            return False
    ...
    def evaluate_rule_node(self, rule_node: Dict, user_data: Dict) -> bool:
        ...
        try:
            ...
            if all(key in rule_node for key in ["字段", "操作符", "值"]):
                ...
                self.logger.info(
                    f"条件评估: {field}({user_value}) {operator} {value} = {result}")
                return result
