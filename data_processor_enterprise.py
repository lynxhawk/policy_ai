...
class DataProcessor:
    ...
    def __init__(self):
        ...
        # 企业相关字段
        self.enterprise_boolean_fields = ['高新技术企业', '是否上市', '国有企业', '民营企业', '外资企业', '科技型企业', '制造业企业', '服务业企业']
        ...
        for key, value in enterprise_dict.items():
            try:
                ...
                # 成立时间字段
                    ...
                    normalized[key] = self.extract_numeric_value(value, 1900, 2030)
                ...
                # 员工数量字段
                    ...
                    normalized[key] = self.extract_numeric_value(value, 0, 1000000)
        ...
        try:
            ...
            if operator == 'between':
                if isinstance(condition_value, list) and len(condition_value) == 2:
                    enterprise_val = self.extract_numeric_value_simple(enterprise_value)
            ...
            self.logger.error(f"条件检查异常: enterprise={enterprise_value}, operator={operator}, condition={condition_value}, 错误: {e}")
            return False
    ...
    def evaluate_rule_node(self, rule_node: Dict, enterprise_data: Dict) -> bool:
        ...
        try:
            ...
            if all(key in rule_node for key in ["字段", "操作符", "值"]):
                ...
                result = self.check_condition(enterprise_value, operator, value)
                
                self.logger.info(f"条件评估: {field}({enterprise_value}) {operator} {value} = {result}")
                return result
            ...
            # 如果是容器节点，处理逻辑运算
                ...
                for i, sub_rule in enumerate(rules):
                    sub_result = self.evaluate_rule_node(sub_rule, enterprise_data)
